...
import 'package:pelanggaran/Page/Admin/Kelas/addkelas.dart';
import 'package:pelanggaran/Page/Admin/Kelas/detailkelas.dart';
...
class KelasData {
  ...
  KelasData({required this.id, required this.kelas, required this.jurusan});

  factory KelasData.fromJson(Map<String, dynamic> json) {
    return KelasData(
      id: json['id'],
      kelas: json['kelas'],
      jurusan: json['jurusan'] == null ? "" : json["jurusan"],
    );
  }
}
...
class _ListKelasState extends State<ListKelas> {
  ...
  void fetchDataKelas(String query) async {
    ...
    try {
      final response = await http.get(
          Uri.parse("https://pelanggaranprestasi.000webhostapp.com/kelasapi"),
          headers: {
            'Cookie': token,
          });
      ...
      if (response.statusCode == 200) {
        ...
        if (decodeData is List) {
          List<KelasData> newDataList =
              decodeData.map((item) => KelasData.fromJson(item)).toList();
          if (mounted) {
            setState(() {
