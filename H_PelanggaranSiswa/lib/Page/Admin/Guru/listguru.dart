...
import 'package:pelanggaran/Page/Admin/Guru/addguru.dart';
import 'package:pelanggaran/Page/Admin/Guru/detailguru.dart';
import 'package:pelanggaran/Page/Admin/dashboardadmin.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:http/http.dart' as http;

class ListGuru extends StatefulWidget {
  const ListGuru({super.key});

  @override
  State<ListGuru> createState() => _ListGuruState();
}
...
class _ListGuruState extends State<ListGuru> {
  ...
  List<GuruData> searchData = [];
  Widget loadingListView = const Center(
    child: CircularProgressIndicator(),
  );

  void fetchDataGuru(String query) async {
    ...
    try {
      final response = await http.get(
          Uri.parse("https://pelanggaranprestasi.000webhostapp.com/guruapi/"),
          headers: {
            'Cookie': token,
          });
      ...
      if (response.statusCode == 200) {
        ...
        if (decodeData is List) {
          List<GuruData> newDataList =
              decodeData.map((item) => GuruData.fromJson(item)).toList();
          if (mounted) {
            setState(() {
